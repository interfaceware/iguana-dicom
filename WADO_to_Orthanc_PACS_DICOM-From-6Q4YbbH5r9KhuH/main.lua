-- Sample WADO interaction function calls --
-- Before proceeding, please refer to README.lua on the left side
-- Includes example how to edit DICOM raw file tags in Translator script itself using dicom.dll. 
-- Function call and 'require' line are commented out, and it has to wait until DLL for Iguana 6.x becomes available. 
local w, RESOURCE = require 'WADOhelpers'
local env = require 'environmentBuilder'

local resources = {'patients','studies','series','instances'}
-- uncomment next line when dicom_raw.dll for Iguana 6.x becomes available
...
function mainWADOexample()
   ...
   if not file2modifyLocally then 
      ...
      -- uncomment next line when dicom.dll for Iguana 6.x becomes available
      -- file2modifyLocally = dicomedit.edit()
   end
