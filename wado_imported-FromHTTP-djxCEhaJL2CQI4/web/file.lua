...
function os.fs.dirExists(Path)
   Path = os.fs.name.XtoNative(Path)
...
function os.fs.name.XtoNative(Path)
   local FilePath = os.fs.Xabspath(Path)
...
function io.Xopen(FileName, Mode)
   FileName = os.fs.name.XtoNative(FileName)
...
function os.fs.cleanDir(Dir, List)
   ...
   local LocalList = {}
   for Name,Info in os.fs.glob(os.fs.name.XtoNative(Dir:addDir())..'*') do
      trace(Name,Info)
      LocalList[Name] = Info
      if (Info.isdir) then
         os.fs.cleanDir(Name)
...
local function XConvertProcessLine(T)
   local Dir = os.fs.name.XtoNative(T.dir)
...
function os.Xexecute(T)
   local CmdString = XConvertProcessLine(T)
   return OsExecute(CmdString)
end

local POpen = io.popen
function io.Xpopen(T)
...
function os.fs.Xabspath(Path)
   return AbsPath(Path):gsub('\\','/')   
end

local Access = os.fs.access
function os.fs.Xaccess(Path)
   Path = os.fs.name.XtoNative(Path)
   return Access(Path)
end
